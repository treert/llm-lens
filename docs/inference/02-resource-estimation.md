...
- **激活**:decode 时只有 $O(\text{batch} \times d)$,几 MB,可忽略;
...
- 这是**上界**。量化模型要额外做反量化计算,且 KV Cache 读取随上下文
  增长而加重,实际打 5~7 折:8B Q4 实际约 60~110 tok/s,对个人使用
  绰绰有余(人阅读速度 < 20 tok/s)。
...
- 出路二:选 MoE 模型(总参数大但激活少,如 30B-A3B),量化后权重更小、
  decode 时按激活参数量读数据,兼顾容量与速度——这是 MoE 对消费卡的
  独特价值。
