...
- [ ] L92 post_attention 的 $g=3.7$ 极端通道与常驻通道 dim 3680:
  ...
  (部分已答 2026-09-18:embed 侧 dim 3680 完全普通,|mean| 排名 6632/7168、std 1.15x 中位;
  lm_head 侧 |mean| 排名 281/7168(5.6 倍中位)且逐 token std 仅 0.38x 中位——近似恒定负列,
  softmax 下功能中性。图景:3680 方向由 trunk 内部产生并停泊,lm_head 用恒定列把它挡在
  logits 外。待验证:down_proj 第 3680 行范数(写回侧)、激活实测)
