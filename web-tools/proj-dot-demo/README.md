...
- **分片流式采样**：采样与矩阵不变量计算都按固定时间片（~24ms/块，按实测自适应
  块大小）切片、片间让出主线程，并把已采样本流式重绘——大维度（H≤512、D≤8192）
  下 UI 不卡，直方图随采样逐步收敛、可随时看出分布形状。
- **暂停 / 重置**：采样中可**暂停**（保留已采样本、按钮变"继续"，点继续接着采）、
  **重置**（清空已采样本回"尚未采样"）。采样过程中改动任何采样参数（方案/H/D/
  样本数/种子/退火/显式，以及方案二/三的 ρ）会**自动重置**——停止并清空，等手动
  点运行；曲线勾选与横轴归一不触发重置（只重绘）。
