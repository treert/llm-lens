/**
 * 瓶颈投影块 y = ABx（单块）长度平方 s = ‖y‖² 的理论密度（纯数学层，无 DOM 依赖）。
 *
 * 设定：x ∈ R^D 为单位向量，B 为 M×D 高斯矩阵（元素方差 1/D 下投），A 为 D×M 高斯
 * 矩阵（元素方差 1/M 上投）。方差配对（fan-in）使单块保长度：E‖ABx‖² = ‖x‖²。
 * M ∈ [1, 4D]（瓶颈维，可小于也可大于 D）。
 *
 * 方案一（固定 x，随机矩阵）——卡方因子化（精确）：
 *   中间层 z = Bx：‖z‖² = varB·χ²_M ~ Gamma(M/2, 2·varB)，均值 M·varB
 *   （B 的元素方差 varB 可选 1/D——配对下投、均值 M/D，或 1/M——均值归一 = 1）。
 *   完整块 y = Az（z = Bx）：条件于 z，‖Az‖² = (‖z‖²/M)·χ²_D，卡方与 ‖z‖² 独立 ⇒
 *     s = χ²_M · χ²_D / (DM)      （两个独立卡方因子）
 *   - 均值恒 1（配对 fan-in 保长度期望）；方差 2/M + 2/D + 4/(MD)
 *   - 两个不同形状伽马的乘积 = 广义 Bessel-K_ν 闭式（ν = (M−D)/2）：
 ...
 *     Var(ln s) = ψ′(M/2)+ψ′(D/2)；中位数 ≈ e^{−(1/M+1/D)} < 1——均值被重尾撑着、
 *     典型样本偏小（"均值-中位数分裂"的单块版）。
 *   （注：abVar/abLogMean/abLogVar/abMedianApprox 仍接受块数 L 参数，本工具用 L=1；
 *    任意 L 的链矩公式见 git 历史。）
 *
 * 方案二（固定矩阵，随机 x 球面均匀，quenched）：
 *   s = xᵀWx，W = PᵀP（P = AB）。球面二次型精确矩：
 *     E_x[s] = trW/D，Var_x(s) = 2[tr(W²) − (trW)²/D] / (D(D+2))
 *   实例中心 trW/D 的跳动（Wick 全方差律）：
 *     中间层（P=B）：Var(trW/D) = 2M·varB²/D（varB = 1/D 时即 2M/D³）
 *     完整块 AB：Var(trW/D) = (2M+4D+2)/(MD²)      （M=D 时 ≈ 6/D²）
 *   随维度消失（trW 是平方和的自平均；对照 proj-dot-demo 的 trM 符号和，涨落不消失）。
 *
