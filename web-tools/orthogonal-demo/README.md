...
- **中位数曲线**：固定 $N$， $\max\rho$（两两点乘最大值 = 最小夹角的余弦）的中位数随
  $K$ 的变化，三条线对比： $F^M$（beta 幂次，全 $K$ 适用）、Gumbel 渐近、
  一阶近似 $\sqrt{2\ln M/N}$（双侧 $\sqrt{2\ln(2M)/N}$）。
- **密度曲线**：给定 $(N, K)$， $\max\rho$ 的分布密度（ $F^M$ 与 Gumbel 渐近两条），
  叠加单对点乘的精确 beta 密度作对比；单侧且 $K$ 小时横轴自动扩展到负半轴。
  右纵轴（概率轴，0~1）叠加两条**近似正交概率**曲线——把 $|\rho| < t$ 当作"正交"：
  单对 $P(|\rho|\le t)$（精确，只依赖 $N$）与 $P(\max|\rho|\le t)$
  （ $K$ 个向量全部两两近似正交的概率，即 $F^M$ 双侧 CDF）。
...
|---|---|---|---|---|---|
| **$F^M$（beta 幂次）** | 蓝实线 | 有限 $K$ 近似（独立性假设） | **$K=2$ 精确**； $K \ge 3$ 误差 ~1~2% | ~1% | 无渐近要求（有限 $N$、 $K$ 即可用） |
| **Gumbel 渐近** | 黄虚线 | 极值极限定理 | 明显失真 | $K \gtrsim 20$ 后 ~2% 以内 | $\ln K = o(N)$ |
| **一阶近似 $\sqrt{2\ln M/N}$（双侧 $\sqrt{2\ln(2M)/N}$）** | 橙实线 | 只留指数主项 | 单侧 $K=2$ 恰为 0（= 真值）；双侧 $K=2$ 仍明显高估 | 系统性**偏高 15~25%**，且收敛极慢 | $\ln K = o(N)$ |
...
两张图除 $\rho$ 主轴外均带**角度副轴**（中位数曲线右侧、密度曲线顶部，
$\theta = \arccos(\rho)$， $\rho = 0$ 对应 90°），tooltip 悬浮也同时显示 $\rho$ 与对应夹角。
密度曲线另有**右侧概率纵轴**（0~1），供两条近似正交概率曲线使用
（tooltip 中该两系列按百分比显示）。

控制区可运行**蒙特卡洛模拟**（设计细节见 `monte-carlo-design.md`）：
顺序逐条轨迹流式推进、时间片可中断，密度曲线叠加点积直方图（贴合单对 beta
绿线）与当前 $K$ 处的 $\max\rho$ 模拟密度（贴合蓝线），中位数曲线叠加跨轨迹中位数与
IQR 带（贴合理论中位数线）；「自动追加」（默认开）持续累积轨迹数 $R$ 收窄
统计涨落，也可手动「追加一条」。首次运行会实测可分配内存并据此确定 $K$ 上限。
...
- 小 $K$ 时与蒙特卡洛高度吻合（实测 $K=2 \sim 128$ 中位数误差均在 1~2% 以内）；
- 大 $K$ 时渐近等价于 §3 的 Gumbel 形式——Gumbel 本就是它的极限，两条线在图上
  随 $K$ 增大逐渐贴合，本身就是对极限定理的直观演示；
- 天然支持负半轴：单侧小 $K$ 时 $\max\rho$ 有可观概率为负， $F_{\mathrm{beta}}$ 在
  $t<0$ 有定义，密度自动显示负侧质量。
...
**分位数函数**（对任意 $p \in (0,1)$；中位数曲线取 $p=0.5$ 即中位数，密度曲线横轴上限取
$p=0.999$）：由 $F = p$ 反解
...
**$\max\rho$ 的密度**（密度曲线的"Gumbel 渐近"线）：对 $R = \sqrt{(W + a)/N}$ 做变量替换，
...
（备查：标准 Gumbel 的均值为 $\mathbb{E}[W] = 2(\gamma + \ln\kappa)$，中位数为
$2\ln\kappa - 2\ln\ln 2$， $\gamma \approx 0.5772$ 为 Euler–Mascheroni 常数；
统计栏使用中位数字径，均值未采用。）

### 4. 一阶近似（中位数曲线的"$\sqrt{2\ln M/N}$"线）

把 $M$ 对点乘当作独立 $\mathcal{N}(0, 1/N)$，用高斯最大值启发式：令
$M \cdot P(\rho > t) \approx 1$（期望超标次数 = 1），只保留指数主项 $e^{-Nt^2/2}$，得

$$\frac{Nt^2}{2} \approx \ln M \quad\Longrightarrow\quad
\max\rho \approx \sqrt{\frac{2\ln M}{N}}, \qquad M = \binom{K}{2}$$

**双侧 $\max|\rho|$** 的超标事件是 $|\rho| > t$：正负两条对称尾巴各算一次机会，
期望超标次数为 $2M \cdot P(\rho > t)$，故 $\ln M$ 换成 $\ln(2M)$：

$$\max|\rho| \approx \sqrt{\frac{2\ln(2M)}{N}}$$

$t^2$ 上恰好差一个 $2\ln 2 / N$ 的平移——与 Gumbel 层 $\kappa$ 双侧翻倍
（§3， $(\max|\rho|)^2$ 平移 $2\ln 2/N$）是同一个 $\ln 2$。旧写法把
$\ln M = 2\ln K - \ln 2$ 中的 $-\ln 2$ 一并丢掉得到 $2\sqrt{\ln K/N}$，
恰好近似等于双侧式；单双侧之分的相对量级为 $\ln 2 / (2\ln M)$，大 $K$ 下可忽略。
...
不过这些口径差异换算到 $\rho$ 轴是 $O(1/(N\rho))$ 量级，远小于一阶近似丢掉
...
$K$ 为个位数时 Gumbel 明显失真（极端例子 $K=2$：只有一对，单侧 $\max\rho$ 的中位数
精确等于 0），且单侧小 $K$ 时 $\max\rho$ 有可观概率取负值。
...
极值才进入 Gumbel 吸引域；实测 $K \gtrsim 20$ 后中位数误差进入 2% 以内。
...
也系统性偏高 15%~25%（中位数曲线上橙线始终压在蓝线上方，缝隙收拢很慢）。
小 $K$ 端：单侧 $K=2$ 时 $M=1$，公式退化为 0，恰好等于真值中位数；
双侧 $K=2$ 给出 $\sqrt{2\ln 2/N} \approx 1.18/\sqrt{N}$，仍明显高于真值中位数
$\approx 0.67/\sqrt{N}$（ $|\rho| \approx |Z|/\sqrt{N}$， $Z$ 为标准正态）。
