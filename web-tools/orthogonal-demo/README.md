...
- **中位数曲线**：固定 N，max ρ（两两点乘最大值 = 最小夹角的余弦）的中位数随 K 的变化，
...
绿线）与当前 K 处的 max ρ 模拟密度（贴合蓝线），中位数曲线叠加跨轨迹中位数与
...
**max ρ 的密度**（密度曲线的"Gumbel 渐近"线）：对 $R = \sqrt{(W + a)/N}$ 做变量替换，
