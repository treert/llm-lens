...
(精确密度正比于 (1-c^2)^((d-3)/2))。本模型的 d=5120,理论 std 约 0.0140。

全表 129280 行的 Gram 需 2·V²·d ≈ 1.7e17 FLOP 与 64 GB 内存,不可行;
...
输出:
- output/ds_v4_1_flash/weight_moments/cosine_dist.json:分位数/均值/std/|cos|>0.1 占比;
- output/ds_v4_1_flash/weight_moments/cosine_dist.npz:直方图计数(供重绘);
- output/ds_v4_1_flash/figures/cosine_dist.png:线性/对数双联图 + 理论曲线。

用法(在仓库根目录下):
    python analysis/ds_v4_1_flash/analyze_cosine_dist.py
    python analysis/ds_v4_1_flash/analyze_cosine_dist.py --sample-rows 4096
"""
...
def main() -> None:
    ...
    xe = collect_rows(pe, EMBED_NAME, args.chunk_rows, sample_idx)
    print("收集 head 行 ...")
    xh = collect_rows(ph, HEAD_NAME, args.chunk_rows, sample_idx)
    ...
    fig.suptitle(f"DeepSeek-V4.1-Flash row cosine distribution ({args.sample_rows} sampled rows each)")
    fig.tight_layout()
