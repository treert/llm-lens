...
K3 的 tie_word_embeddings=false,embed_tokens 与 lm_head 各自训练。本脚本回答:
1. 配对余弦:同一 token 的嵌入行与 LM Head 行的余弦相似度分布
   (基线:随机两向量的余弦 ~ N(0, 1/7168),std 约 0.0118);
2. 各向异性:词嵌入的均值向量是否显著(余弦到均值方向的分布);
3. 范数极端 token:嵌入范数最小/最大的 token 是谁(经 tiktoken.model 解码);
4. 两个表的范数是否联动:e_norm 与 h_norm 的逐 token 相关。

输出:
- output/kimi_k3/weight_moments/embed_lmhead.npz:逐 token 的 e_norm/h_norm/pair_cos/cos_to_mean;
- output/kimi_k3/weight_moments/embed_lmhead.json:分布分位数、极端 token 清单;
- output/kimi_k3/figures/embed_lmhead.png:四联图。

用法(在仓库根目录下):
    python analysis/kimi_k3/analyze_embed_lmhead.py
"""
...
def load_vocab(model_dir: Path) -> dict[int, str]:
    """解析 tiktoken.model(每行 'base64 token rank'),失败时回退为 <id N>。"""
    path = model_dir / "tiktoken.model"
    vocab: dict[int, str] = {}
    try:
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if len(parts) != 2:
                    continue
                tok = base64.b64decode(parts[0])
                vocab[int(parts[1])] = tok.decode("utf-8", errors="replace")
    except FileNotFoundError:
        print("未找到 tiktoken.model,token 只显示 id")
    return vocab
...
def main() -> None:
    ...
    r = pass1(pe, ne, ph, nh, args.chunk_rows, sample_idx)
    ...
    cos_me = cos_to_direction(pe, ne, args.chunk_rows, mean_e_dir)
    cos_mh = cos_to_direction(ph, nh, args.chunk_rows, mean_h_dir)
    ...
    print(f"随机基线: mean={triu.mean():.5f} std={triu.std():.5f}(理论 {1/math.sqrt(7168):.5f})")
